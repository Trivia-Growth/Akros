...
| Story | Título | Descrição | Owner | Status | Spec | Concluída | Commit |
|-------|--------|-----------|-------|--------|------|-----------|--------|
| E13-S09 | Sair do mock: adapters restantes | **Replanejada em 31/08.** Backend de lead operacional em 04/09: `crm.leads` + RLS, RPC atômica lead→cliente e `lead-capturar` v2 aplicados; `curl` provou `201` pela function, `401` direto e `413` acima de 8 KiB. Captura pública real ficou separada da gestão admin ainda mock. AC restantes seguem nas ondas E13-S10/S11. | @codex | 🟨 | ✅ | — | — |
| E13-S10 | Onda 0: as 5 tabelas que faltam | `0014` modela `crm.propostas`, `configuracoes.integracoes`, `configuracoes.contas_agenda`, `configuracoes.contas_canal`, `configuracoes.equipe` e compartilhamentos. RLS FORCE admin-only, auditoria e guardas anti-segredo passaram em Postgres 17 limpo e remoto. `configuracoes` foi exposto no Data API; admin leu 6/6 e escreveu/atualizou, cliente recebeu `[]` e escrita `403`. Registro técnico ficou soft-deletado. **Bloqueia os dois P0 restantes**: sem elas o "store fictícia fora do ar" é inalcançável, porque `ConfiguracoesPage` sozinha depende de quatro. **Arquitetural** | @codex | 🟩 | ✅ | 2026-09-05 | — |
| E13-S11 | Ondas 1 a 4: migração por dependência | Plano canônico e inventário remoto em 05/09: folhas → ancoradas em `cliente_id` → telas que cruzam contexto → desligar store e deletar `MAPA_ID_REAL_PARA_MOCK`. **Programas, Configurações, Agenda, transcrições, Jornada, Documentos, Pagamentos, Mensagens, Dashboard e Perfil portal; Propostas, Comunicação, Dashboard, Operação, Cliente 360 e Fila de revisão admin entregues em leitura:** adapters usam RLS por `auth.uid()`, preservam UUID real e normalizam JSONB/numeric de borda. Perfil consulta a única linha RLS, sem depender do claim legado. Receita fica separada por moeda; Operação agrupa gargalo por título/responsável, não por UUID de fixture. Cliente 360 recompõe cada jornada apenas com fases/etapas do mesmo UUID. Mapa temporário foi removido; layouts deixam chunks mock exclusivos do demo e E2E serial 10/10 prova que sessão real não baixa `mocks/store`. Bootstrap compartilha refresh em `StrictMode` para respeitar teto remoto. Corte fechado em 05/09 (AC-4 a AC-6): `/admin/leads`, `/admin/aprovacoes`, `/admin/pagamentos` e `/admin/reativacao` passam a existir só no demo — fora dele redirecionam ao dashboard e somem do menu, e o e2e AC-7 prova pelo caminho do usuário que sessão real não carrega a store. Os dois P0 do SECURITY_DEBT fecham aqui. Configurações bloqueia credenciais até Vault; criação de reunião, transição de etapa, upload/assinatura, comprovante/conciliação, mensageria e ciclo de proposta esperam endpoints seguros. `0015` liberou INSERT só para admin. Regra fixa: adapter + hooks + telas na mesma entrega, nunca `di.ts` sozinho. Migração real das 4 telas ocultadas depende de RPC/Edge Function de transição/auditoria e vira story própria. **Arquitetural** | @codex | 🟩 | ✅ | 2026-09-05 | — |
| E13-S12 | Agente WhatsApp real: Evolution + OpenRouter | `0016`, Vault e duas Edge Functions entregam configuração BYOK pela UI: API keys não voltam ao browser, capability token vai em header, webhook é registrado pela própria função, entrada é deduplicada e agente fica desligado por padrão. Prompt sem tools encaminha pedido jurídico/financeiro/humano; falha externa não reenvia para não duplicar WhatsApp. PostgreSQL 17 limpo, `deno check`, 155 testes e build passaram. Falta somente rollout DevOps de migration/functions antes do primeiro uso. **Arquitetural** | @codex | 🟨 | ✅ | — | — |
