...
| E13-S03 | Schema `documentos` + `pagamentos` | Réplica do padrão. `dados_recebimento` é o primeiro caso "não é dado de cliente" (qualquer autenticado lê, só admin escreve) — RLS provado nas duas direções, inclusive releitura confirmando que UPDATE bloqueado não mudou a linha (204 sozinho não prova isso). | @claude-code | 🟩 | ✅ |
...
| E13-S05 | Schema `comunicacao` | 5 tabelas. Mensagens/threads viram JSONB (lidas como bloco, não cruzadas entre conversas). Caso novo: `regras_atendimento_ia`/`fontes_conhecimento` são **admin-only** (zero policy de cliente — `[]` na consulta, não erro). RLS provado via PostgREST. | @claude-code | 🟩 | ✅ |
| E13-S06 | `audit.*` append-only em todas as tabelas de E13-S01..S05 | Trigger genérico (`SECURITY DEFINER`, `search_path` fixo) nas 17 tabelas. Append-only garantido por GRANT (nunca UPDATE/DELETE pra ninguém) — RLS sozinha não bastaria porque `service_role` tem `BYPASSRLS`. Provado: `service_role` tentando `UPDATE`/`DELETE` em `audit.eventos` recebe 403 de privilégio de verdade. | @claude-code | 🟩 | ✅ |
| E13-S07 | Schema `lgpd.*` (consentimento, export, delete) | 2 tabelas — cliente cria/lê a própria solicitação, só admin processa. Cobertas por `audit.*`. RLS provado (incluindo releitura confirmando bloqueio real). Automação de export/delete de fato fica pra quando houver dado real de cliente. | @claude-code | 🟩 | ✅ |
| E13-S08 | Frontend: trocar `MockClienteRepository` por adapter Supabase | Primeiro bounded context a sair do mock de verdade — prova que o padrão porta/adapter (ADR-0002) aguenta a troca sem reescrever UI. Escopo estreito: só `useClienteAtivo` (portal) e `Clientes360Page`/`Cliente360` (admin) — as 6 telas que criam cliente a partir de lead ficam mock (dependem de `crm.leads`, ainda não existe). Mapa temporário id-uuid↔id-mock confinado no adapter (SPEC_DEVIATION, fecha em E13-S09). Verificado ao vivo: admin vê as 2 linhas reais, edição persiste (reread via `service_role`), portal do Carlos mostra dado real cruzado com jornada/documentos/pagamentos ainda mock. | @claude-code | 🟩 | ✅ |
