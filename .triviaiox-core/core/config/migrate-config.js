 ...
 *   - .triviaiox-core/project-config.yaml (L2) — team-shared project settings
 *   - ~/.triviaiox/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.triviaiox/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.triviaiox-core/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, ".triviaiox-core", "core-config.yaml");
  const frameworkPath = path.join(projectRoot, ".triviaiox-core", "framework-config.yaml");
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, ".triviaiox-core", "core-config.yaml");
  const backupPath = path.join(projectRoot, ".triviaiox-core", "core-config.backup.yaml");
...
/**
 * Ensure the ~/.triviaiox/ directory exists.
 *
 * @returns {string} Path to ~/.triviaiox/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), ".triviaiox");
...
/**
 * Write user config to ~/.triviaiox/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), ".triviaiox", "user-config.yaml");
...
/**
 * Write project config to .triviaiox-core/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, ".triviaiox-core", "project-config.yaml");
