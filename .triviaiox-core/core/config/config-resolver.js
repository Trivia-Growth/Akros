 ...
 */
const CONFIG_FILES = {
  framework: ".triviaiox-core/framework-config.yaml",
  project: ".triviaiox-core/project-config.yaml",
  pro: "pro/pro-config.yaml",
  local: ".triviaiox-core/local-config.yaml",
  legacy: ".triviaiox-core/core-config.yaml",
  user: path.join(os.homedir(), ".triviaiox", "user-config.yaml"),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  if (options.appDir) {
    const appConfigPath = path.join(options.appDir, "triviaiox-app.config.yaml");
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.triviaiox/user-config.yaml)
...
function loadLegacyConfig(projectRoot) {
  ...
  const suppressDeprecation =
    process.env.TRIVIAIOX_SUPPRESS_DEPRECATION === "true" ||
    process.env.TRIVIAIOX_SUPPRESS_DEPRECATION === "1";

  if (!suppressDeprecation) {
    warnings.push(
      "[DEPRECATION] Monolithic core-config.yaml detected. " +
        "Run `triviaiox config migrate` to split into layered config files. " +
        "Monolithic format will be removed in v4.0.0. " +
        "Set TRIVIAIOX_SUPPRESS_DEPRECATION=true to silence this warning.",
    );
  }
...
function getConfigAtLevel(projectRoot, level, options = {}) {
  ...
  switch (level) {
    ...
    case "L3":
      ...
      relativePath = path.join(options.appDir, "triviaiox-app.config.yaml");
      break;
    case "local":
    case "4":
    case "L4":
...
/**
 * Ensure the ~/.triviaiox/ directory exists with secure permissions.
 *
...
/**
 * Set a value in the user config file (~/.triviaiox/user-config.yaml).
