...
const MONITOR_SERVER_URL = process.env.TRIVIAIOX_MONITOR_URL || "http://localhost:4001/events";
...
class DashboardEmitter {
  ...
  constructor() {
    ...
    this.fallbackPath = path.join(this.projectRoot, ".triviaiox", "dashboard", "events.jsonl");
  ...
  async emit(type, data = {}) {
    ...
    const event = {
      ...
      triviaiox_agent: this.currentAgent || undefined,
      triviaiox_story_id: this.currentStoryId || undefined,
      data,
    };
      ...
      const response = await fetch(MONITOR_SERVER_URL, {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            triviaiox_agent: event.triviaiox_agent,
            triviaiox_story_id: event.triviaiox_story_id,
          },
        }),
