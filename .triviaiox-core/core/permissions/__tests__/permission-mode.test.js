...
describe("PermissionMode", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "triviaiox-test-"));
    await fs.mkdir(path.join(tempDir, ".triviaiox"), { recursive: true });
  ...
  describe("load()", () => {
    ...
    it("should load mode from config file", async () => {
      await fs.writeFile(
        path.join(tempDir, ".triviaiox", "config.yaml"),
        "permissions:\n  mode: auto\n",
      );
