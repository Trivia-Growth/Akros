...
class PermissionMode {
  ...
  constructor(projectRoot = process.cwd()) {
    ...
    this.configPath = path.join(projectRoot, ".triviaiox", "config.yaml");
  ...
  async _saveToConfig(mode) {
    ...
    // Ensure .triviaiox directory exists
    const triviaioxDir = path.dirname(this.configPath);
    await fs.mkdir(triviaioxDir, { recursive: true });
