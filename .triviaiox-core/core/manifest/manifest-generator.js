 ...
 * Scans the .triviaiox-core directory structure to build comprehensive manifests.
 *
...
class ManifestGenerator {
  constructor(options = {}) {
    ...
    this.triviaioxCoreDir = path.join(this.basePath, ".triviaiox-core");
    this.manifestDir = path.join(this.triviaioxCoreDir, "manifests");
    this.version = "2.1.0";
  }
  ...
  async generateAgentsManifest() {
    const agentsDir = path.join(this.triviaioxCoreDir, "development", "agents");
        ...
        try {
          ...
          if (parsed && parsed.agent) {
            ...
            agents.push({
              ...
              status: "active",
              file_path: `.triviaiox-core/development/agents/${file}`,
  ...
  async generateTasksManifest() {
    const tasksDir = path.join(this.triviaioxCoreDir, "development", "tasks");
      ...
      for (const file of mdFiles) {
        try {
          ...
          tasks.push({
            ...
            file_path: `.triviaiox-core/development/tasks/${file}`,
            status: "active",
          });
        } catch (e) {
          errors.push(`Error parsing ${file}: ${e.message}`);
