...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--help" || arg === "-h") {
      ...
      args.interval = Number.parseInt(argv[++i], 10);
    } else if (arg.startsWith("--interval=")) {
      args.interval = Number.parseInt(arg.split("=")[1], 10);
...
/**
 * Write HTML graph to .triviaiox/graph.html and open in default browser.
...
function handleHtmlOutput(graphData, options = {}) {
  const outputDir = path.resolve(process.cwd(), ".triviaiox");
 ...
 * Writes to .triviaiox/graph.dot, .triviaiox/graph.mmd, or .triviaiox/graph.html.
 * @param {Object} args - Parsed CLI args
...
async function handleWatch(args) {
  ...
  const registryPath = path.resolve(process.cwd(), ".triviaiox-core/data/entity-registry.yaml");
...
function handleHelp() {
  const usage = `
Usage: triviaiox graph [command] [options]
...
Examples:
  triviaiox graph --deps                        Show dependency tree
  triviaiox graph --deps --format=json          Output as JSON
  triviaiox graph --deps --format=html          Interactive HTML graph (opens browser)
  triviaiox graph --deps --watch                Live DOT file for VS Code preview
  triviaiox graph --deps --watch --format=html  Live HTML with auto-refresh
  triviaiox graph --deps --watch --format=mermaid  Live Mermaid file
  triviaiox graph --deps --watch --interval=10  Refresh every 10 seconds
  triviaiox graph --stats                       Show entity stats and cache metrics
`.trim();
...
async function handleSummary(args) {
  ...
  sections.push("TRIVIAIOX Graph Dashboard");
