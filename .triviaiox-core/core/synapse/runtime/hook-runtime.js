...
function getStaleSessionTTL(cwd) {
  try {
    ...
    const configPath = path.join(cwd, ".triviaiox-core", "core-config.yaml");
...
function resolveHookRuntime(input) {
  ...
  try {
    const { loadSession, createSession, cleanStaleSessions } = require(
      path.join(cwd, ".triviaiox-core", "core", "synapse", "session", "session-manager.js"),
    );
    const { SynapseEngine } = require(
      path.join(cwd, ".triviaiox-core", "core", "synapse", "engine.js"),
    );
