/**
 * Verification Tests for Active TRIVIAIOX Memory Modules
 *
...
test("merge-rules.yaml exists in .triviaiox", () => {
  const rulesPath = path.join(process.cwd(), ".triviaiox", "merge-rules.yaml");
  const exists = fs.existsSync(rulesPath);

  assertTrue(exists, "merge-rules.yaml should exist in .triviaiox");
});
