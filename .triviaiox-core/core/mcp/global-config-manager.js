 ...
 *
 * Manages the global MCP configuration at ~/.triviaiox/mcp/
...
/**
 * Check if global TRIVIAIOX directory exists
 * @returns {boolean}
 */
function globalDirExists() {
  return fs.existsSync(getGlobalTriviaioxDir());
}
...
function addServer(serverName, serverConfig = null) {
  ...
  if (!config) {
    return { success: false, error: 'Global config not found. Run "triviaiox mcp setup" first.' };
  }
