 ...
 * @returns {string} Path to .triviaiox-core/tools/mcp
 */
function getProjectMcpPath(projectRoot = process.cwd()) {
  return path.join(projectRoot, ".triviaiox-core", "tools", "mcp");
}
...
function createLink(projectRoot = process.cwd(), options = {}) {
  ...
  if (!fs.existsSync(globalPath)) {
    return {
      success: false,
      error: 'Global MCP directory does not exist. Run "triviaiox mcp setup" first.',
      linkPath,
      globalPath,
    };
  }
