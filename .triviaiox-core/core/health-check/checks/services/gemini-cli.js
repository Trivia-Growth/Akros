 ...
 *
 * @module triviaiox-core/health-check/checks/services/gemini-cli
 * @version 1.0.0
...
class GeminiCliCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      // Check for rules.md (TRIVIAIOX rules)
      try {
      ...
      // Check for TRIVIAIOX agents
      try {
        const agentsDir = path.join(geminiDir, "rules", "TRIVIAIOX", "agents");
        const agentFiles = await fs.readdir(agentsDir);
        details.triviaioxAgents = agentFiles.filter((f) => f.endsWith(".md")).length;
      } catch {
        details.triviaioxAgents = 0;
      }
    } catch {
      // No project config
    }
    ...
    if (details.projectConfig && details.triviaioxAgents === 0) {
      warnings.push("No TRIVIAIOX agents installed for Gemini CLI");
    }
    ...
    if (issues.length > 0) {
      return this.warning(`Gemini CLI needs attention: ${issues.join(", ")}`, {
        recommendation: issues.includes("Not authenticated")
          ...
          : "Run `npx triviaiox-core install` and select Gemini CLI",
    ...
    if (warnings.length > 0) {
      return this.warning(`Gemini CLI configuration incomplete: ${warnings.join(", ")}`, {
        recommendation: "Run `npx triviaiox-core install` and select Gemini CLI",
    ...
    if (details.triviaioxAgents > 0) parts.push(`${details.triviaioxAgents} TRIVIAIOX agents`);
