 ...
 *
 * @module triviaiox-core/health-check/checks/local/environment-vars
 * @version 1.0.0
...
/**
 * TRIVIAIOX-specific environment variables
 */
const TRIVIAIOX_VARS = ["TRIVIAIOX_DEBUG", "TRIVIAIOX_LOG_LEVEL"];
...
class EnvironmentVarsCheck extends BaseCheck {
  ...
  async execute(_context) {
    ...
    // Check TRIVIAIOX-specific vars (info only)
    for (const varName of TRIVIAIOX_VARS) {
      if (process.env[varName]) {
        triviaioxVars.push({ name: varName, value: this.maskValue(process.env[varName]) });
      }
    }
