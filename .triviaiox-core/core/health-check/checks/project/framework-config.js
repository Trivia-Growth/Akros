 ...
 *
 * Verifies that TRIVIAIOX framework configuration files are present.
 *
 * @module triviaiox-core/health-check/checks/project/framework-config
 * @version 1.0.0
...
const REQUIRED_CONFIGS = [
  { path: ".triviaiox-core", type: "directory", description: "TRIVIAIOX core framework" },
...
const RECOMMENDED_CONFIGS = [
  { path: ".triviaiox", type: "directory", description: "TRIVIAIOX local configuration" },
...
class FrameworkConfigCheck extends BaseCheck {
  constructor() {
    super({
      ...
      description: "Verifies TRIVIAIOX framework configuration is present",
      domain: CheckDomain.PROJECT,
      severity: CheckSeverity.HIGH,
      timeout: 3000,
      cacheable: true,
      healingTier: 0, // Cannot auto-fix framework setup
      tags: ["triviaiox", "config", "framework"],
    });
  }
  ...
  async execute(context) {
    ...
    if (missingRequired.length > 0) {
      ...
      return this.fail(`Missing required framework configuration: ${missing}`, {
        recommendation: "Run TRIVIAIOX setup or manually create missing directories",
        details: {
    ...
    if (missingRecommended.length > 0) {
      ...
      return this.warning(`Missing recommended configuration: ${missing}`, {
        recommendation:
          "Consider adding recommended configuration for full TRIVIAIOX functionality",
        details: {
