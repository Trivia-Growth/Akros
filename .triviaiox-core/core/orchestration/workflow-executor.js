...
class WorkflowExecutor {
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.workflowPath = path.join(
      projectRoot,
      ".triviaiox-core/development/workflows/development-cycle.yaml",
    );
    this.statePath = path.join(projectRoot, ".triviaiox/workflow-state/");
    this.configPath = path.join(projectRoot, ".triviaiox-core/core-config.yaml");
    ...
    for (const line of lines) {
      ...
      if (severityMatch) {
        issues.push({
          ...
          line: fileMatch ? Number.parseInt(fileMatch[2], 10) : 0,
