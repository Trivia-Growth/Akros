...
async function createContextFile(context, outputDir = os.tmpdir()) {
  ...
  const contextPath = path.join(outputDir, `triviaiox-context-${Date.now()}.json`);
...
async function spawnInline(agent, task, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn("bash", [scriptPath, ...args], {
      env: {
        ...process.env,
        TRIVIAIOX_DEBUG: opts.debug ? "true" : "false",
        TRIVIAIOX_OUTPUT_DIR: opts.outputDir,
        TRIVIAIOX_INLINE_MODE: "true", // Signal to pm.sh that we're running inline
      },
...
async function cleanupOldFiles(outputDir = os.tmpdir(), maxAgeMs = 3600000) {
  ...
  try {
    ...
    const triviaioxFiles = files.filter(
      (f) =>
        f.startsWith("triviaiox-output-") ||
        f.startsWith("triviaiox-lock-") ||
        f.startsWith("triviaiox-context-"),
    );

    for (const file of triviaioxFiles) {
...
function formatCompatibilityReport(report) {
  const lines = [
    "═══════════════════════════════════════════════════════════════",
    "                 TRIVIAIOX Terminal Spawner Compatibility Report     ",
    "═══════════════════════════════════════════════════════════════",
    "",
