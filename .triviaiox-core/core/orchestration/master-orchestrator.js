 ...
 *
 * Features:
 * - AC1: Located in `.triviaiox-core/core/orchestration/`
...
class MasterOrchestrator extends EventEmitter {
  ...
  async resumeFromEpic(fromEpic) {
    ...
    for (const epicNumStr of Object.keys(this.executionState.epics)) {
      const epicNum = Number.parseInt(epicNumStr, 10);
      if (epicNum >= fromEpic) {
        this.executionState.epics[epicNum] = {
          status: EpicStatus.PENDING,
          startedAt: null,
  ...
  _getCompletedGates() {
    return Object.entries(this.executionState.epics)
      ...
      .map(([num, _]) => Number.parseInt(num, 10));
  }
  ...
  async loadState(storyId = null) {
    ...
    const targetPath = targetStoryId
      ? path.join(this.projectRoot, ".triviaiox", "master-orchestrator", `${targetStoryId}.json`)
      : this.statePath;
  ...
  async findLatestValidState() {
    const stateDir = path.join(this.projectRoot, ".triviaiox", "master-orchestrator");
