...
`;
    ...
    report += "\n---\n\n*Generated by TRIVIAIOX Build Orchestrator v1.0.0*\n";
...
async function main() {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    console.log(`
${chalk.bold("Build Orchestrator")} - TRIVIAIOX Autonomous Build (Story 8.5)
...
`);
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--dry-run") {
      ...
      options.globalTimeout = Number.parseInt(args[++i], 10);
