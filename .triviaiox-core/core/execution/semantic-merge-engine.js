 ...
 * Loads project-specific merge rules from .triviaiox/merge-rules.yaml
 */
class CustomRulesLoader {
  constructor(rootPath = process.cwd()) {
    this.rootPath = rootPath;
    this.rulesPath = path.join(rootPath, ".triviaiox", "merge-rules.yaml");
    this.cache = {
      rules: null,
      lastLoad: null,
  ...
  async executeHook(hookName, context = {}) {
    ...
    try {
      execSync(hookCommand, {
        ...
        env: {
          ...process.env,
          TRIVIAIOX_MERGE_HOOK: hookName,
          TRIVIAIOX_MERGE_CONTEXT: JSON.stringify(context),
        },
      });
      return true;
    } catch (error) {
...
class SemanticMergeEngine extends EventEmitter {
  constructor(config = {}) {
    ...
    // Storage
    this.storageDir = config.storageDir || path.join(this.rootPath, ".triviaiox", "merge");
  }
