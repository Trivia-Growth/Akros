 ...
 *
 * @module triviaiox-core/doctor/fix-handler
 * @story INS-4.1
 */
...
const FIX_MAP = {
  ...
  "agent-memory": {
    ...
    async apply(result, { projectRoot }) {
      const agentsDir = path.join(projectRoot, ".triviaiox-core", "development", "agents");
      let created = 0;
      ...
      for (const agent of EXPECTED_AGENTS) {
        ...
        if (!fs.existsSync(memoryPath)) {
          ...
          fs.writeFileSync(
            memoryPath,
            `# ${agent} Agent Memory\n\n_Created by triviaiox doctor --fix_\n`,
          );
          created++;
        }
      }
  ...
  "claude-md": {
    ...
    async apply() {
      return "CLAUDE.md regeneration requires npx triviaiox-core install --force";
    },
  },
  ...
  "settings-json": {
    ...
    async apply(result, { frameworkRoot }) {
      ...
      return "settings.json regeneration requires npx triviaiox-core install --force";
    },
  },
};
