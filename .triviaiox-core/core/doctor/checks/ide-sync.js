 ...
 *
 * Validates agents in .claude/commands/TRIVIAIOX/agents/ match
 * .triviaiox-core/development/agents/ (count and names).
 *
 * @module triviaiox-core/doctor/checks/ide-sync
 * @story INS-4.1
 */
...
async function run(context) {
  const agentsSourceDir = path.join(
    context.projectRoot,
    ".triviaiox-core",
    "development",
    "agents",
  );
  const agentsIdeDir = path.join(context.projectRoot, ".claude", "commands", "TRIVIAIOX", "agents");
  ...
  if (!fs.existsSync(agentsSourceDir)) {
    return {
      ...
      fixCommand: "npx triviaiox-core install --force",
    };
  }
  ...
  if (!fs.existsSync(agentsIdeDir)) {
    return {
      check: name,
      status: "WARN",
      message: "IDE agents directory not found (.claude/commands/TRIVIAIOX/agents/)",
      fixCommand: "npx triviaiox-core install --force",
    };
  }
