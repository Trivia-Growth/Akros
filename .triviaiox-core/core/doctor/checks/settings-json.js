 ...
 *
 * @module triviaiox-core/doctor/checks/settings-json
 * @story INS-4.1
 */
...
function checkBoundaryAlignment(context, denyRules) {
  const configPath = path.join(context.projectRoot, ".triviaiox-core", "core-config.yaml");
...
async function run(context) {
  ...
  if (denyCount < 40) {
    return {
      ...
      fixCommand: "triviaiox doctor --fix",
    };
  }
