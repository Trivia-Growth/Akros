 ...
 *
 * Facade class that integrates triviaiox-master with the IDS Entity Registry for
 ...
 *
 * @example
 *   const { RegistryLoader } = require('.triviaiox-core/core/ids/registry-loader');
 *   const { IncrementalDecisionEngine } = require('.triviaiox-core/core/ids/incremental-decision-engine');
 *   const { RegistryUpdater } = require('.triviaiox-core/core/ids/registry-updater');
 *   const { FrameworkGovernor } = require('.triviaiox-core/core/ids/framework-governor');
 *
 ...
 *
 * Story: IDS-7 (triviaiox-master IDS Governor Integration)
 */
  ...
  async preCheck(intent, entityType) {
    ...
    return this._withTimeout(
      async () => {
        ...
        const topMatch = analysis.recommendations.length > 0 ? analysis.recommendations[0] : null;
  ...
  async impactAnalysis(entityId) {
    ...
    return this._withTimeout(
      async () => {
        ...
        const indirectConsumers = [...allAffected].filter((id) => !directConsumers.includes(id));
    ...
    return this._withTimeout(
      async () => {
        const task = {
          ...
          agent: metadata.agent || "triviaiox-master",
  ...
  async getStats() {
    return this._withTimeout(
      async () => {
        ...
        const healthScore =
          allEntities.length > 0 ? Math.round((verifiedCount / allEntities.length) * 100) : 0;
