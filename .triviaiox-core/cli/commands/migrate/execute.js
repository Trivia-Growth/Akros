 ...
 */
async function createModuleDirectories(triviaioxCoreDir, options = {}) {
 ...
 */
async function migrateModule(moduleData, moduleName, triviaioxCoreDir, options = {}) {
...
async function executeMigration(plan, options = {}) {
  ...
  if (!dryRun) {
    await createModuleDirectories(plan.triviaioxCoreDir, { onProgress });
  }
  ...
  for (const moduleName of moduleNames) {
    ...
    const moduleResult = await migrateModule(moduleData, moduleName, plan.triviaioxCoreDir, {
      verbose,
      onProgress,
      dryRun,
    });
  ...
  if (plan.uncategorized.length > 0 && !dryRun) {
    ...
    for (const file of plan.uncategorized) {
      try {
        const targetPath = path.join(plan.triviaioxCoreDir, "core", file.relativePath);
        await copyFileWithMetadata(file.sourcePath, targetPath);
        result.totalFiles++;
        result.totalSize += file.size;
      } catch (error) {
  ...
  if (!dryRun && cleanupOriginals) {
    ...
    for (const [_moduleName, config] of Object.entries(MODULE_MAPPING)) {
      for (const dir of config.directories) {
        const originalDir = path.join(plan.triviaioxCoreDir, dir);
...
async function saveMigrationState(projectRoot, state) {
  const statePath = path.join(projectRoot, ".triviaiox-migration-state.json");
