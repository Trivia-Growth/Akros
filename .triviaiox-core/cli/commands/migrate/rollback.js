 ...
 */
async function removeV21Structure(triviaioxCoreDir, options = {}) {
...
async function restoreFromBackup(backup, projectRoot, options = {}) {
  ...
  const backupTriviaioxCoreDir = path.join(backup.path, ".triviaiox-core");

  // Restore .triviaiox-core files
  ...
  for (const file of backup.manifest.files) {
    try {
      const sourcePath = file.isConfig
        ...
        : path.join(backupTriviaioxCoreDir, file.relativePath);

      const targetPath = file.isConfig
        ? path.join(projectRoot, file.relativePath)
        : path.join(triviaioxCoreDir, file.relativePath);
...
async function executeRollback(projectRoot, options = {}) {
  ...
  const triviaioxCoreDir = path.join(projectRoot, ".triviaiox-core");
  result.removal = await removeV21Structure(triviaioxCoreDir, { onProgress });
  ...
  for (const dir of expectedDirs) {
    if (fs.existsSync(path.join(triviaioxCoreDir, dir))) {
      validCount++;
    }
  }
...
function formatRollbackSummary(result) {
  ...
  lines.push("");
  lines.push("🔙 TRIVIAIOX Migration Rollback");
  lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
