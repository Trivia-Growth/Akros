 ...
 * @param {string} triviaioxCoreDir - Path to .triviaiox-core
 * @returns {Promise<Object>} Validation result
 */
async function validateStructure(triviaioxCoreDir) {
  const result = {
    valid: true,
    modules: {},
    errors: [],
  ...
  for (const moduleName of expectedModules) {
    const moduleDir = path.join(triviaioxCoreDir, moduleName);
    const moduleResult = {
      exists: false,
      fileCount: 0,
      totalSize: 0,
  ...
  const rootEntries = await fs.promises.readdir(triviaioxCoreDir, { withFileTypes: true });
...
async function runLintCheck(projectRoot, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      ...
      if (errorMatch) result.errors = Number.parseInt(errorMatch[1], 10);
      if (warningMatch) result.warnings = Number.parseInt(warningMatch[1], 10);
...
async function runTests(projectRoot, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      ...
      if (passedMatch) result.passedCount = Number.parseInt(passedMatch[1], 10);
      if (failedMatch) result.failed = Number.parseInt(failedMatch[1], 10);
      if (totalMatch) result.total = Number.parseInt(totalMatch[1], 10);
...
async function runFullValidation(projectRoot, options = {}) {
  ...
  result.structure = await validateStructure(triviaioxCoreDir);
  ...
  result.imports = await verifyImports(triviaioxCoreDir);
...
function generateSummary(migrationResult, validationResult, options = {}) {
  ...
  lines.push("  3. Report issues: github.com/triviaiox/issues");
  lines.push("");
  lines.push("To rollback: triviaiox migrate --rollback");
