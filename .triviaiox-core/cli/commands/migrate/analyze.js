...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(triviaioxCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: "No .triviaiox-core directory found",
    };
  }
  ...
  const hasV21Structure = v21Modules.every((module) =>
    fs.existsSync(path.join(triviaioxCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some((dir) =>
    fs.existsSync(path.join(triviaioxCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: "Unable to detect TRIVIAIOX version structure",
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .triviaiox-core
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(triviaioxCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(triviaioxCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(triviaioxCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(triviaioxCoreDir, moduleName);
