...
function createBackupDirName() {
  ...
  return `.triviaiox-backup-${dateStr}`;
}
...
/**
 * Create backup of .triviaiox-core directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  // Check if .triviaiox-core exists
  if (!fs.existsSync(triviaioxCoreDir)) {
    throw new Error("No .triviaiox-core directory found. Is this an TRIVIAIOX v2.0 project?");
  }
  ...
  const files = await getAllFiles(triviaioxCoreDir);
  const manifest = {
    version: "2.0",
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(triviaioxCoreDir, srcFile);
    const destFile = path.join(backupDir, ".triviaiox-core", relativePath);
  ...
  const configFiles = [
    "triviaiox.config.js",
    "triviaiox.config.json",
    ".triviaiox/config.yaml",
    ".mcp.json",
  ];
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error("Backup manifest not found. Is this a valid TRIVIAIOX backup?");
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, ".triviaiox-core", file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter((entry) => entry.isDirectory() && entry.name.startsWith(".triviaiox-backup-"))
    .map((entry) => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace(".triviaiox-backup-", ""),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith(".triviaiox-backup-")) {
