...
function printHeader(fromVersion, toVersion) {
  console.log("");
  console.log(`🔄 TRIVIAIOX Migration v${fromVersion} → v${toVersion}`);
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log("");
}
...
async function runMigration(options) {
  ...
  if (!versionInfo.isV2) {
    ...
    console.error(`❌ ${versionInfo.error || "Cannot detect TRIVIAIOX version"}`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (!migrationResult.success) {
      ...
      console.log("  Run: triviaiox migrate --rollback");
      process.exit(1);
    }
  ...
  try {
    const triviaioxCoreDir = path.join(projectRoot, ".triviaiox-core");
    const importResult = await updateAllImports(triviaioxCoreDir, plan, { verbose, onProgress });
  ...
  const backupDir = `.triviaiox-backup-${new Date().toISOString().split("T")[0]}`;
...
async function runRollbackCommand(options) {
  ...
  console.log("");
  console.log("🔙 TRIVIAIOX Migration Rollback");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log("");
...
async function showBackupStatus(_options) {
  ...
  console.log("");
  console.log("📦 TRIVIAIOX Migration Backups");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
...
function createMigrateCommand() {
  ...
  migrate
    .description("Migrate TRIVIAIOX from v2.0 to v4.0.4 modular structure")
...
Migration Flow:
  Phase 1: Backup
    - Creates .triviaiox-backup-{date}/ with all files
...
Examples:
  $ triviaiox migrate                          # Interactive migration
  $ triviaiox migrate --dry-run                # Preview changes
  $ triviaiox migrate --verbose                # Detailed output
  $ triviaiox migrate -y --skip-tests          # Non-interactive, skip tests
  $ triviaiox migrate --rollback               # Restore from backup
  $ triviaiox migrate --status                 # Show backups

Rollback:
  If migration fails or you want to revert:
  $ triviaiox migrate --rollback

  Manual rollback:
  $ cp -r .triviaiox-backup-{date}/* .triviaiox-core/
`,
    );
