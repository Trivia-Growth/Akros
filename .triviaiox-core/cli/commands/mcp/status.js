...
async function executeStatus(options) {
  ...
  if (globalExists) {
    ...
    console.log('  Run "triviaiox mcp setup" to create global config');
  }
  ...
  switch (linkStatus.status) {
    ...
    case LINK_STATUS.NOT_LINKED:
      ...
      console.log('  Run "triviaiox mcp link" to link project to global config');
      break;
    case LINK_STATUS.BROKEN:
      console.log("  Status: 🔴 Broken link");
      console.log(`  Target: ${linkStatus.target}`);
      console.log('  Run "triviaiox mcp link --force" to fix');
      break;
    case LINK_STATUS.DIRECTORY:
      ...
      if (projectConfig.found) {
        ...
        console.log('  Run "triviaiox mcp link --migrate" to use global config');
      }
      break;
    default:
  ...
  // Quick actions
  if (!globalExists) {
    console.log("\n💡 Quick start: triviaiox mcp setup --with-defaults");
  } else if (linkStatus.status === LINK_STATUS.NOT_LINKED) {
    console.log("\n💡 Link project: triviaiox mcp link");
  } else if (serverList.enabled === 0) {
    console.log("\n💡 Add server: triviaiox mcp add context7");
  }
}
