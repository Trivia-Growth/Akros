...
async function executeAdd(server, options) {
  ...
  if (options.listTemplates) {
    ...
    console.log("Usage: triviaiox mcp add <server-name>");
    console.log("Example: triviaiox mcp add context7");
    return;
  }
  ...
  if (!globalConfigExists()) {
    ...
    console.log('Run "triviaiox mcp setup" first to create global configuration.');
    process.exit(1);
  }
  ...
  if (!server) {
    ...
    console.log("  triviaiox mcp add myserver --type sse --url https://example.com/mcp");
    console.log('  triviaiox mcp add myserver --command npx --args "-y,@scope/mcp-server"');
    process.exit(1);
  }
  ...
  if (result.success) {
    ...
    if (result.error.includes("No template available")) {
      ...
      console.log(`  triviaiox mcp add ${server} --type sse --url <url>`);
      console.log(`  triviaiox mcp add ${server} --command npx --args "-y,@scope/server"`);
    }
