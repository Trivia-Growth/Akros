 ...
 * MCP Setup Command
 *
 * Creates the global ~/.triviaiox/mcp/ directory structure.
 *
...
function createSetupCommand() {
  ...
  setup
    .description("Create global MCP configuration structure at ~/.triviaiox/")
...
async function executeSetup(options) {
  ...
  console.log(`  ${getGlobalTriviaioxDir()}/`);
  console.log("  ├── mcp/");
  ...
  console.log("\nNext steps:");
  console.log('  1. Run "triviaiox mcp link" in your project to use global config');
  console.log('  2. Run "triviaiox mcp add <server>" to add more servers');
  console.log('  3. Run "triviaiox mcp status" to check configuration');
