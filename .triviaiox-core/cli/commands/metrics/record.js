  ...
  record
    ...
    .action(async (options) => {
      try {
        ...
        const layerNum = Number.parseInt(options.layer, 10);
        ...
        const result = {
          passed,
          durationMs: options.duration ? Number.parseInt(options.duration, 10) : 0,
          findingsCount: options.findings ? Number.parseInt(options.findings, 10) : 0,
          metadata: {},
        };
        ...
        if (layerNum === 2) {
          if (options.coderabbit) {
            result.coderabbit = {
              findingsCount:
                Number.parseInt(options.crCritical, 10) +
                Number.parseInt(options.crHigh, 10) +
                Number.parseInt(options.crMedium, 10) +
                Number.parseInt(options.crLow, 10),
              severityBreakdown: {
                critical: Number.parseInt(options.crCritical, 10),
                high: Number.parseInt(options.crHigh, 10),
                medium: Number.parseInt(options.crMedium, 10),
                low: Number.parseInt(options.crLow, 10),
              },
            };
          }
          ...
          if (options.quinn) {
            result.quinn = {
              findingsCount: Number.parseInt(options.quinnFindings, 10),
