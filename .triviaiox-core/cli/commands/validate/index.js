...
function createValidateCommand() {
  ...
  validate
    .description("Validate TRIVIAIOX-Core installation integrity")
...
async function runValidation(options) {
  ...
  // Check if TRIVIAIOX-Core is installed
    ...
    if (options.json) {
      console.log(
        JSON.stringify(
          {
            status: "failed",
            error: "TRIVIAIOX-Core not found in current directory",
            // SECURITY: Sanitize path - only show relative indicator
            location: ".triviaiox-core",
          },
          null,
          2,
        ),
      ...
      console.error(chalk.dim(`Expected at: ${triviaioxCoreDir}`));
      console.error(chalk.dim("\nRun `npx triviaiox-core install` to install TRIVIAIOX-Core"));
    }
  ...
  if (sourceDir) {
    const sourceManifest = path.join(sourceDir, ".triviaiox-core", "install-manifest.yaml");
  ...
  if (!sourceDir && options.repair) {
    ...
    const possibleSources = [
      ...
      path.join(projectRoot, "node_modules/triviaiox-core"),
      path.join(projectRoot, "node_modules/triviaiox-core"),
    ];

    for (const src of possibleSources) {
      if (fs.existsSync(path.join(src, ".triviaiox-core", "install-manifest.yaml"))) {
        sourceDir = src;
        break;
      }
    }
