...
// In framework-dev: __dirname = triviaiox-core/.triviaiox-core/cli/commands/pro → ../../../../pro/license
// In project-dev: pro is installed via npm as @triviaiox-fullstack/pro
function resolveLicensePath() {
  ...
  // 2. Try node_modules/@triviaiox-fullstack/pro/license (project-dev mode)
  try {
    const proPkg = require.resolve("@triviaiox-fullstack/pro/package.json");
    ...
    // @triviaiox-fullstack/pro not installed via npm
  }

  // 3. Try project root node_modules (fallback)
  const projectRoot = process.cwd();
  const cwdPath = path.join(projectRoot, "node_modules", "@triviaiox-fullstack", "pro", "license");
...
function loadLicenseModules() {
  try {
    ...
    console.error("TRIVIAIOX Pro license module not available.");
    console.error("Install TRIVIAIOX Pro: npm install @triviaiox-fullstack/pro");
    process.exit(1);
  }
}

/**
 * Get TRIVIAIOX Core version from package.json
 */
function getTriviaioxCoreVersion() {
  try {
...
async function activateAction(options) {
  ...
  if (!key) {
    ...
    console.error("Usage: triviaiox pro activate --key PRO-XXXX-XXXX-XXXX-XXXX");
    process.exit(1);
  }
  ...
  console.log("\nActivating TRIVIAIOX Pro license...");
  console.log(`Key: ${maskKey(key)}`);
  console.log("");

  try {
    const machineId = generateMachineId();
    const triviaioxCoreVersion = getTriviaioxCoreVersion();

    const result = await licenseApi.activate(key, machineId, triviaioxCoreVersion);
    ...
    const proSourceDir = path.join(projectRoot, "node_modules", "@triviaiox-fullstack", "pro");
    ...
    if (fs.existsSync(proSourceDir)) {
      ...
      if (scaffoldProContent) {
        ...
        if (scaffoldResult.success) {
          ...
          console.error('Try running "triviaiox pro activate" again to retry scaffolding.');
        }
        console.log("");
      }
    } else {
      console.log("Note: @triviaiox-fullstack/pro package not found in node_modules.");
...
function statusAction() {
  ...
  if (!cache) {
    ...
    console.log("  Activate: triviaiox pro activate --key PRO-XXXX-XXXX-XXXX-XXXX");
    console.log("  Purchase: https://triviagrowth.ai/pro");
    console.log("");
    return;
  }
  ...
  if (info && info.inGrace) {
    ...
    console.log("  Please revalidate your license: triviaiox pro validate");
  }
...
async function deactivateAction(options) {
  ...
  if (!options.force) {
    console.log("\nDeactivating TRIVIAIOX Pro License");
    console.log(`Key: ${maskKey(cache.key)}`);
    console.log("\nThis will:");
  ...
  try {
    ...
    console.log("");
    console.log("To reactivate: triviaiox pro activate --key <KEY>");
    console.log("");
  } catch (error) {
...
async function validateAction() {
  ...
  console.log("\nValidating TRIVIAIOX Pro license...\n");
...
// ---------------------------------------------------------------------------
// triviaiox pro setup (AC-12: Install-gate)
// ---------------------------------------------------------------------------

/**
 * Setup and verify @triviaiox-fullstack/pro installation.
 *
 * Since @triviaiox-fullstack/pro is published on the public npm registry,
...
async function setupAction(options) {
  ...
  if (options.verify) {
    // Verify-only mode
    console.log("Verifying @triviaiox-fullstack/pro installation...\n");

    try {
      const { execSync } = require("child_process");
      const result = execSync("npm ls @triviaiox-fullstack/pro --json", {
        stdio: "pipe",
        timeout: 15000,
      });
      const parsed = JSON.parse(result.toString());
      const deps = parsed.dependencies || {};
      if (deps["@triviaiox-fullstack/pro"]) {
        console.log(
          `✅ @triviaiox-fullstack/pro@${deps["@triviaiox-fullstack/pro"].version} is installed`,
        );
      } else {
        console.log("❌ @triviaiox-fullstack/pro is not installed");
        console.log("");
        console.log("Install with:");
        console.log("  npm install @triviaiox-fullstack/pro");
      }
    } catch {
      console.log("❌ @triviaiox-fullstack/pro is not installed");
      console.log("");
  ...
  // Install mode
  console.log("@triviaiox-fullstack/pro is available on the public npm registry.");
  console.log("No special tokens or configuration needed.\n");

  console.log("Installing @triviaiox-fullstack/pro...\n");

  try {
    const { execSync } = require("child_process");
    execSync("npm install @triviaiox-fullstack/pro", {
      stdio: "inherit",
      timeout: 120000,
    });
    console.log("\n✅ @triviaiox-fullstack/pro installed successfully!");
  } catch (error) {
  ...
  console.log("  triviaiox pro activate --key PRO-XXXX-XXXX-XXXX-XXXX");
  console.log("");
  console.log("To check license status:");
  console.log("  triviaiox pro status");
  console.log("");
  console.log("Documentation: https://triviagrowth.ai/pro/docs");
  console.log("");
}
...
/**
 * Create the `triviaiox pro` command with all subcommands.
 * @returns {Command}
 */
function createProCommand() {
  const proCmd = new Command("pro").description("TRIVIAIOX Pro license management");
  ...
  proCmd
    .command("setup")
    .description("Install and verify @triviaiox-fullstack/pro")
