...
// Resolve core config modules (relative from .triviaiox-core/cli/commands/config/)
...
function migrateAction(options) {
  ...
  try {
    ...
    if (originalStr === resolvedStr) {
      ...
      console.log(
        "This may be expected due to key normalization. Run `triviaiox config diff` to inspect.",
      );
    }
...
function splitL1(config) {
  ...
  l1.metadata = { name: "TriviaGrowth Triviaiox", framework_version: "3.12.0" };
...
function initLocalAction() {
  const root = getProjectRoot();
  const templatePath = path.join(root, ".triviaiox-core", "local-config.yaml.template");
  const targetPath = path.join(root, ".triviaiox-core", "local-config.yaml");

  if (!fs.existsSync(templatePath)) {
    console.error("Template not found: .triviaiox-core/local-config.yaml.template");
    process.exit(1);
  }
  ...
  console.log("Created: .triviaiox-core/local-config.yaml (from template)");
...
/**
 * Create the `triviaiox config` command with all subcommands.
 * @returns {Command}
 */
