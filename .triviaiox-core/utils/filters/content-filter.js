...
function stripHtml(html) {
  ...
  clean = clean.replace(
    /<\/?(div|p|br|h[1-6]|li|tr|td|th|blockquote|pre|section|article)[^>]*>/gi,
    "\n",
  );
...
function filterContent(input, options = {}) {
  ...
  const reductionPct =
    originalLength > 0 ? Math.round((1 - filteredLength / originalLength) * 100) : 0;
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === "--max-tokens" && argv[i + 1]) {
      args.max_tokens = Number.parseInt(argv[i + 1], 10);
      i++;
