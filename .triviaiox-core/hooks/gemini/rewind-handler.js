#!/usr/bin/env node
/**
 * TRIVIAIOX Rewind Handler
 * Story GEMINI-INT.14 - /rewind Integration
 *
 * Syncs Gemini's /rewind command with TRIVIAIOX state.
 */
...
async function handleRewind() {
  ...
  // Sync with TRIVIAIOX memory layer
  try {
    const memoryDir = path.join(projectDir, ".triviaiox", "memory");
    ...
    const logDir = path.join(projectDir, ".triviaiox", "logs");
