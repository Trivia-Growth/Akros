#!/usr/bin/env node
/**
 * TRIVIAIOX Session End Hook for Gemini CLI
 * Story GEMINI-INT.6 - TRIVIAIOX Hooks System
 *
...
async function sessionEnd() {
  ...
  try {
    const sessionDir = path.join(projectDir, ".triviaiox", "sessions");
    ...
    let modifiedFiles = [];
    const sessionFilesPath = path.join(projectDir, ".triviaiox", "session-files.json");
    ...
    let toolCount = 0;
    const toolLogPath = path.join(projectDir, ".triviaiox", "logs", "tool-results.jsonl");
