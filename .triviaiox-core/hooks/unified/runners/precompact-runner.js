 ...
 *
 * Execution flow:
 * 1. Detect triviaiox-pro availability via pro-detector
...
async function onPreCompact(context) {
  try {
    // 1. Detect triviaiox-pro availability
    const proAvailable = isProAvailable();

    if (!proAvailable) {
      console.log("[PreCompact] triviaiox-pro not available, skipping session digest");
    ...
    setImmediate(async () => {
      try {
        // Dynamic import from triviaiox-pro
        ...
        if (!digestModule || typeof digestModule.extractSessionDigest !== "function") {
          console.error("[PreCompact] Digest extractor not found or invalid in triviaiox-pro");
          return;
        }
