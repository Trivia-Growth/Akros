 ...
 */
const EVENT_MAPPING = {
  // TRIVIAIOX Event -> { claude: ClaudeEvent, gemini: GeminiEvent }
...
class UnifiedHook {
   ...
   * @param {string} config.event - TRIVIAIOX event name
  ...
  toClaudeConfig() {
    ...
    // Runners not yet implemented - path .triviaiox-core/hooks/unified/runners/ does not exist
    return null;

    // return {
    //   event: claudeEvent,
    //   matcher: this.matcher,
    //   handler: `node .triviaiox-core/hooks/unified/runners/claude-runner.js ${this.name}`,
    // };
  }
  ...
  toGeminiConfig() {
    ...
    //       name: this.name,
    //       type: 'command',
    //       command: `node .triviaiox-core/hooks/unified/runners/gemini-runner.js ${this.name}`,
