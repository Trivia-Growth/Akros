...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .triviaiox-core/development/scripts/unified-activation-pipeline.js devops
...
commands:
  ...
  - name: health-check
    visibility: [full, quick, key]
    description: 'Run unified health diagnostic (triviaiox doctor --json + governance interpretation)'
  - name: sync-registry
    visibility: [full, quick, key]
    args: '[--full] [--heal]'
...
dependencies:
  ...
  repository_agnostic_design:
    ...
    installation_modes:
      framework-development: '.triviaiox-core/ is SOURCE CODE (committed to git)'
      project-development: '.triviaiox-core/ is DEPENDENCY (gitignored, in node_modules)'
    detection_priority:
      - '.triviaiox-installation-config.yaml (explicit user choice)'
  ...
  git_authority:
    ...
    enforcement_mechanism: |
      ...
      - Checks $TRIVIAIOX_ACTIVE_AGENT environment variable
      - Blocks push if agent != "devops"
      - Displays helpful message redirecting to @devops
      - Works in ANY repository using TRIVIAIOX-FullStack
...
---
---
*TRIVIAIOX Agent - Synced from .triviaiox-core/development/agents/devops.md*
