...
export function PropostaDocumentoRealPage() {
  const { id } = useParams<{ id: string }>();
  const { propostas, contatos, carregando, erro } = usePropostasSupabase();

  if (carregando) return <EstadoDocumento mensagem="Carregando proposta real…" />;
  if (erro) return <EstadoDocumento mensagem="Não foi possível carregar a proposta." />;

  const proposta = propostas.find((item) => item.id === id);
  const contato = contatos.find((item) => item.id === proposta?.leadOuClienteId);
  if (!proposta) return <EstadoDocumento mensagem="Proposta não encontrada." />;

  return (
    <div className="flex flex-col gap-5" data-testid="proposta-documento-real">
...
function EstadoDocumento({ mensagem }: { mensagem: string }) {
  return (
    <div className="mx-auto max-w-md py-16 text-center">
      <p className="text-sm text-ink-muted">{mensagem}</p>
